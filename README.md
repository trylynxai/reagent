...
### Example: Katerpillar Integration

The [Katerpillar](https://github.com/anomalyco/katerpillar) research discovery agent demonstrates a full ReAgent integration. It captures LLM calls, tool executions, and failures during automated research workflows.

#### What Gets Traced

- **LLM Calls**: Model, provider, prompt, response, token counts, cost
- **Tool Calls**: Source connectors (arXiv, HackerNews, GitHub, Reddit, web search)
- **Run Failures**: Error messages and tracebacks when runs fail

#### Running the Example

```bash
# Terminal 1: Start ReAgent server
cd reagent
pip install -e ".[server]"
export REAGENT_SERVER_DB=./reagent/traces.db
reagent server start --port 8080

# Terminal 2: Start ReAgent dashboard
cd reagent/dashboard
npm install
npm run dev -- --port 5173

# Terminal 3: Start Katerpillar backend (with ReAgent tracing)
cd examples/katerpillar/katerpillar/backend
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
export REAGENT_DB_PATH=./reagent/traces.db
uvicorn server:app --host 0.0.0.0 --port 8001

# Terminal 4: Start Katerpillar frontend
cd examples/katerpillar/katerpillar/frontend
npm install
PORT=3001 REACT_APP_BACKEND_URL=http://localhost:8001 npm start
```
...
| Service | URL |
|---------|-----|
| Katerpillar UI | http://localhost:3001 |
| ReAgent Dashboard | http://localhost:5173 |
| ReAgent API | http://localhost:8080 |
| Katerpillar API | http://localhost:8001 |

#### Data Persistence

The ReAgent database is stored at `./reagent/traces.db` (relative to the backend directory). This location persists across service restarts and system reboots.

To backup your traces:
```bash
cp ./reagent/traces.db ./reagent/traces_backup_$(date +%Y%m%d).db
```

To reset traces (optional):
```bash
rm ./reagent/traces.db  # Next run will create a fresh database
```

#### Viewing Traces

1. Open http://localhost:5173 to see the ReAgent dashboard
2. The "Overview" tab shows aggregate stats (total runs, tokens, cost)
3. The "Runs" tab lists all discovery runs with their status
4. Click a run to see the trace graph with LLM calls and tool executions
5. Failed runs show the error message and stack trace

#### Instrumentation Architecture

```
Katerpillar Backend
├── emergent_compat.py    → Records LLM calls via record_llm_call()
├── source_connectors.py   → Records tool/source calls via record_tool_call()
├── server.py              → Wraps run_discovery_task() with start_trace/end_trace
└── reagent_instrumentation.py → ReAgent client setup and utilities
```
